...
class YawPredictor:
    def __init__(self, device: str = 'cpu') -> None:
        ...
        state_url = "https://cloud.ovgu.de/s/TewGC9TDLGgKkmS/download/6DRepNet360_Full-Rotation_300W_LP+Panoptic.pth"
        #state_url = "https://invalideurl.com/6DRepNet360_Full-Rotation_300W_LP+Panoptic.pth"
        saved_state_dict = load_state_dict_from_url(state_url, model_dir='pretrained_models/SixDRepNet360', map_location=device)    
    ...
    def predict_yaws(self, images):
        self.model.eval()
        with torch.no_grad():
            images = torch.tensor(images)
            images = self.transformations(images)
            R_pred = self.model(images.to(self.device))
