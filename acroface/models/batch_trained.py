...
from acroface.evaluation.performance import PerformanceCollection, EvaluationMetric, setup_performance
...
@dataclass
class MiniBatchTrainedModelConfig:
...
class MiniBatchTrainedModel:
    def __init__(self, config: MiniBatchTrainedModelConfig, rng: random.Random = None) -> None:
        self.config = config
        if rng is None:
            rng = random.Random()
        self.rng = rng
    ...
    def minibatch_train(self, 
                        *,
                        ...
                        experiment_tracker: ExperimentTracker,
                        training_config: MiniBatchTrainedModelConfig,
                        scheduler=None):
        initial_performance = setup_performance(training_config.evaluation_metrics)
        epoch = 0
        ...
        if training_config.do_pre_eval:
            best_performance, best_model_reference, _ = self.evaluate_model(best_performance=best_performance,
        ...
        for epoch in trange(training_config.max_epochs, desc='Epochs'):
            ...
            for i, batch in enumerate(tqdm(training_dataset, desc='Training batch')):
                ...
                if ((training_config.eval_time is not None
                    ...
                    best_performance, best_model_reference, no_improvement_count = self.evaluate_model(best_performance=best_performance,
                                                                            best_model_reference=best_model_reference,
    ...
    def evaluate_model(self,
                    ...
                    scheduler=None,
                    keep_snapshots='best'):
        gathered_evaluation_results , precictions = model.evaluate_dataset(evaluation_dataset)
